...
import {
  ...
  getNoiseSuppression,
  getMicrophoneProcessingEnabled, saveMicrophoneProcessingEnabled,
  getAutoGainControl, getEchoCancellation, saveAutoGainControl, saveEchoCancellation,
  getStreamQuality,
...
export const AppSettingsScreen = ({ activitySharingEnabled, maxScreenShareQuality, onActivitySharingChange, onClose }: Props) => {
  ...
  const [processingEnabled, setProcessingEnabled] = useState(getMicrophoneProcessingEnabled)
  const [noiseSuppression, setNoiseSuppression] = useState(getNoiseSuppression)
  const [autoGainControl, setAutoGainControl] = useState(getAutoGainControl)
  const [echoCancellation, setEchoCancellation] = useState(getEchoCancellation)
  ...
  const changeQuality = (next: StreamQualityId) => { if (isStreamQualityAllowed(next, maxScreenShareQuality)) { setQuality(next); saveStreamQuality(next) } }
  ...
  const Toggle = ({ checked, disabled = false, description, label, onChange }: { checked: boolean; disabled?: boolean; description: string; label: string; onChange: (enabled: boolean) => void }) => <label className="app-settings__toggle"><span><strong>{label}</strong><small>{description}</small></span><input checked={checked} disabled={disabled} onChange={(event) => onChange(event.target.checked)} type="checkbox" /></label>

  return <main className="app-settings-screen"><header><div><p className="eyebrow">PREFERÊNCIAS DO APP</p><h1>Configurações.</h1><p>Defina os padrões usados sempre que você entrar em uma call.</p></div><button onClick={onClose} type="button"><Icon name="x" /> Fechar</button></header><div className="app-settings__layout"><nav aria-label="Seções das configurações"><button className={section === 'audio' ? 'is-active' : ''} onClick={() => setSection('audio')} type="button"><Icon name="audio" /><span><strong>Áudio</strong><small>Entrada e saídas</small></span></button><button className={section === 'video' ? 'is-active' : ''} onClick={() => setSection('video')} type="button"><Icon name="camera" /><span><strong>Vídeo</strong><small>Câmera padrão</small></span></button><button className={section === 'transmission' ? 'is-active' : ''} onClick={() => setSection('transmission')} type="button"><Icon name="screen" /><span><strong>Transmissão</strong><small>Qualidade e overlay</small></span></button><button className={section === 'privacy' ? 'is-active' : ''} onClick={() => setSection('privacy')} type="button"><Icon name="eye" /><span><strong>Privacidade</strong><small>Atividade visível</small></span></button></nav><section className="app-settings__content">{section === 'audio' && <><div className="app-settings__heading"><span><Icon name="audio" /></span><div><h2>Áudio</h2><p>Escolha os dispositivos que serão usados por padrão nas próximas calls.</p></div></div><div className="app-settings__card"><DeviceSelect kind="audioinput" label="Microfone" preference="inputId" /><DeviceSelect kind="audiooutput" label="Saída de voz" preference="voiceOutputId" /><DeviceSelect kind="audiooutput" label="Saída de compartilhamento" preference="screenOutputId" /><Toggle checked={processingEnabled} description="Desligue para captar o microfone sem ganho automático, redução de ruído ou cancelamento de eco. Ao religar, suas escolhas anteriores são restauradas." label="Pós-processamento do microfone" onChange={(enabled) => { setProcessingEnabled(enabled); saveMicrophoneProcessingEnabled(enabled) }} /><Toggle disabled={!processingEnabled} checked={processingEnabled && noiseSuppression} description="Reduz ruído de fundo. Desligue se cortar trechos da sua voz." label="Redução de ruído" onChange={changeNoise} /><Toggle disabled={!processingEnabled} checked={processingEnabled && autoGainControl} description="Ajusta o volume automaticamente. Deixe desligado se o volume oscilar ao falar perto." label="Ganho automático" onChange={(enabled) => { setAutoGainControl(enabled); saveAutoGainControl(enabled) }} /><Toggle disabled={!processingEnabled} checked={processingEnabled && echoCancellation} description="Evita que o áudio dos alto-falantes volte para a call. Com fones, você pode desligar." label="Cancelamento de eco" onChange={(enabled) => { setEchoCancellation(enabled); saveEchoCancellation(enabled) }} /><p className="app-settings__note">Para voz mais natural, ganho automático e redução de ruído começam desligados. Suas escolhas ficam salvas.</p></div></>}
