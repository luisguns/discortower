...
import { MAX_DIRECT_MESSAGE_IMAGE_SIZE, deleteDirectMessage, listDirectMessages, markDirectConversationRead, resolveDirectMessageImage, searchSocialUser, sendDirectImage, sendDirectText, socialAction } from '../../services/social'
...
export const FriendsWorkspace = ({ profile, overview, onRefresh }: Props) => {
  ...
  return <section className="social-workspace">
    ...
    <section className="social-main">
      ...
      </div> : conversation && friend ? <DirectConversation key={conversation.id} currentUserId={profile.userId} friend={friend} isReadOnly={isReadOnly} loading={loadingMessages} messages={messages} onDelete={removeMessage} onImage={sendImage} onRemove={() => setConfirm({ action: 'remove_friend', friend })} onReRequest={() => void act('send_request', friend.userId)} onSend={sendText} onBlock={() => setConfirm({ action: 'block_user', friend })} fileRef={fileRef} /> : <div className="social-empty"><span><Icon name="chat" /></span><p className="eyebrow">CONVERSAS PRIVADAS</p><h2>Seu círculo, por perto.</h2><p>Selecione um amigo ou encontre alguém pelo @username.</p><button className="social-primary" onClick={() => setTab('add')} type="button">ADICIONAR AMIGO</button></div>}
...
const DirectConversation = ({ currentUserId, friend, isReadOnly, messages, loading, onSend, onImage, onDelete, onRemove, onReRequest, onBlock, fileRef }: { currentUserId: string; friend: FriendProfile; isReadOnly: boolean; messages: DirectMessage[]; loading: boolean; onSend: (text: string) => Promise<boolean>; onImage: (file: File) => Promise<boolean>; onDelete: (message: DirectMessage) => void; onRemove: () => void; onReRequest: () => void; onBlock: () => void; fileRef: RefObject<HTMLInputElement | null> }) => {
