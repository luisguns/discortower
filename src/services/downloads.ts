export const WINDOWS_LTS_RELEASE_URL = 'https://github.com/luisguns/discortower/releases/latest'
...
export const downloadWindowsLts = async () => {
  try {
    ...
    const installer = assets.find((asset) => asset.name === 'splotys-LTS-Windows-x64.exe')
      ...
      ?? assets.find((asset) => asset.name === 'splotys-LTS-Windows-x64.exe')
      ?? assets.find((asset) => /^splotys-Setup-[\w.-]+-x64\.exe$/.test(asset.name ?? ''))
