alter table public.profiles
  add column if not exists name_animation text not null default 'none'
    check (name_animation in ('none', 'breathe', 'spark', 'float'));
...
drop function if exists public.update_my_profile(text, text, text, text, text, smallint, text, text, text);
...
begin
  ...
  if p_name_font not in ('mono', 'condensed', 'rounded', 'serif')
    ...
    or p_name_badge not in ('none', 'soft', 'outline', 'pill')
    or p_name_animation not in ('none', 'breathe', 'spark', 'float') then
  ...
  set display_name = regexp_replace(trim(p_display_name), '\s+', ' ', 'g'),
      ...
      name_animation = p_name_animation
...
revoke all on function public.update_my_profile(text, text, text, text, text, smallint, text, text, text, text) from public, anon, service_role;
grant execute on function public.get_my_access_context() to authenticated, service_role;
grant execute on function public.update_my_profile(text, text, text, text, text, smallint, text, text, text, text) to authenticated;
grant update (name_animation) on public.profiles to authenticated;
