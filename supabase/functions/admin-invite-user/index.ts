...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: invitation, error: insertError } = await client
      .from('invitations')
      .insert({ created_by: user.id, email_normalized: email, expires_at: expiresAt, invited_user_id: invited.user.id, role: requestedRole })
      .select('id,email_normalized,status,role,created_at,expires_at,accepted_at')
      .single()
    ...
    const { error: roleError } = await client.from('profiles').update({ role: requestedRole }).eq('user_id', invited.user.id)
    if (roleError) {
      await client.from('invitations').delete().eq('id', invitation.id)
      await client.auth.admin.deleteUser(invited.user.id)
      throw new Error('INVITATION_ROLE_RECORD_FAILED')
    }
    await writeAudit(client, { action: 'invitation_created', actorUserId: user.id, result: 'success', metadata: { invitationId: invitation.id, role: requestedRole } })
